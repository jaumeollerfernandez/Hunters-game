class Person extends GameObject{  //Creamos una clase Person a partir de la clase GameObject (herencia) para identificar a las personas del juego
    constructor(config){
        ...
        this.aux = "";
        this.lowlatido = document.getElementById('lowlatido');
        this.mediumlatido = document.getElementById('mediumlatido');
        this.highlatido = document.getElementById('audio1');
        this.letsplay = false;
        // this.globalsound.play();
        this.volume = 0
        //utils.setVolume(this.globalsound, this.volume);
        ...
        if (behavior.type === "walk") {
            ...
            if (state.map.isSpaceTaken(this.x, this.y, this.direction)) {
                ...
                if(this.x/16 == 11 && this.y/16 == 8){
                    this.aux = document.getElementById('encontradagrabadora');
                    this.aux.play();
            ...
            if((this.x/16 >=0 && this.x/16 <=3) || (this.x/16 >=17)){
                this.mediumlatido.pause();
                this.lowlatido.pause();
                //this.highlatido.play()
            } 

            if((this.x/16 >=4 && this.x/16 <=7) || (this.x/16 <=16 && this.x/16 >=15)){
                this.highlatido.pause();
                this.lowlatido.pause();
                this.mediumlatido.play();
            }


            if((this.x/16 >=8 && this.x/16 <=14) ){
                this.highlatido.pause();
                this.mediumlatido.pause();
                if(this.y/16 <= 8){
                    this.highlatido.pause();
